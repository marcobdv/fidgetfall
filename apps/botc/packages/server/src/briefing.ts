...
function scriptSheet(view: GameView): string {
  ...
  if (view.script.characters.some((c) => c.id === 'atheist')) {
    lines.push(
      '',
      'Read the ATHEIST on that list again, because it changes what this game is. If it is in',
      'play there are **no evil players at all** — no Demon, no Minions — and everything that',
      'looks like evil is the Storyteller breaking the rules on purpose: false readings, deaths',
      'in the night, numbers that do not add up. The town wins one way only, by executing the',
      '**Storyteller**. `nominate --player Storyteller` is a legal move on this script, on any',
      'day, and it needs the same votes as any execution.',
      '',
      'And it may not be in play. The Storyteller can put a Drunk in that slot instead and run a',
      'perfectly ordinary game, and you will not be told which. So every strange thing has two',
      'explanations from the first morning on. That doubt is the script. Spending an execution',
      'on the Storyteller when you are wrong costs you a day and a townsfolk you could have',
      'tested; never spending it when you are right loses you the game outright.',
    );
  }
...
function writeStorytellerBriefing(view: GameView, room: Room, kind: 'agent' | 'human'): string {
  return [
    ...
    '',
    'Only when the Atheist is on the script and you have actually dealt it. Then: there is no',
    'evil in this game, every piece of it is yours to invent, and the town wins by executing',
    'you. The licence is real and it is total — you may break any rule. It is not a licence to',
    'be arbitrary. Three things hold it together.',
    '',
    'First, **simulate a game that could have existed.** Invent a Poisoner and poison the same',
    'seat all night, every night, the way a real one would. Kill in a pattern a real Demon',
    'would kill in. Do not invent effects that are on no script anywhere — the town is trying',
    'to reason, and reasoning has to be possible for them to arrive anywhere.',
    '',
    'Second, **the announced bag is a lie and the record is not.** `deal` takes announce_counts',
    'so the town hears a normal setup; the truth is written to your own log in the same breath',
    'and comes out at the reveal. Lie to the town in the square, never to the record.',
    '',
    'Third, **a bargain binds you.** You may go to players privately and offer them something',
    'else to win by. If you do, honour it exactly as offered, even when the ending is untidy —',
    '`end_game` takes also_won and names them at the roll call. A deal you wriggle out of is',
    'worse than one you never made, and it is the one thing here nobody can appeal.',
    '',
    'Aim to be executed late rather than never. A town that never puts you up has not been',
    'beaten, it has been bored — if nobody has said your name by day three, the game you are',
    'running is not giving them enough to be suspicious of.',
    '',
