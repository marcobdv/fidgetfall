...
describe('claims', () => {
  it('lets you tell two people two different things', () => {
    ...
    assert.equal(ben.seats.find((s) => s.name === 'Ana')?.claimToYou?.[0]?.id, 'seer');
    assert.equal(cal.seats.find((s) => s.name === 'Ana')?.claimToYou?.[0]?.id, 'baker');
  ...
  it('tells you when someone told you one thing and the town another', () => {
    ...
    assert.equal(anaSeat?.claimToYou?.[0]?.id, 'seer');
    assert.equal(anaSeat?.publicClaim?.[0]?.id, 'baker');
  ...
  it('gives you your own ledger of what you have told whom', () => {
    ...
    assert.deepEqual(
      own.you?.claimsMade?.map((c) => [c.toName, c.characters[0]?.id]),
      [['Ben', 'seer'], ['Cal', 'baker']],
    );
  ...
  it('shows the Storyteller every story a player is running', () => {
    ...
    assert.deepEqual(
      st.seats.find((s) => s.name === 'Ana')?.claimsMade?.map((c) => [c.toName, c.characters[0]?.id]),
      [['Ben', 'seer'], [null, 'baker']],
    );
  });
  ...
  it('replaces the standing claim to that audience, not the others', () => {
    ...
    expectOk(t.game.claim(ana, ['baker'], TOWN));
    assert.deepEqual(
      t.game.claimsMadeBy(ana).map((c) => c.characterIds[0]),
      ['smith', 'baker'],
    );
  });
